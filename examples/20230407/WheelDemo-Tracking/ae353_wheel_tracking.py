...
class Simulator:
    ...
    def step(self, controller):
        ...
        measured_wheel_angle = self.get_sensor_measurements()

        # Get the wheel torque command (run the controller)
        wheel_torque_command = controller.run(self.t, measured_wheel_angle)
        ...
        self.data['measured_wheel_angle'].append(measured_wheel_angle)
        for key in self.variables_to_log:
